...
export function Roadmap() {
  ...
  useEffect(() => {
    if (profile) {
      // Basic auto-completion logic based on level for demo
  ...
  return (
    <div className="max-w-4xl mx-auto animate-fade-in space-y-12 pb-20 px-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-12">
         {[
           { label: 'Current Phase', value: ROADMAP_NODES[activeNode - 1]?.title, icon: Star, color: 'text-indigo-600' },
           { label: 'Total Sync Points', value: `${profile?.xp || 0} XP`, icon: Zap, color: 'text-amber-500' },
           { label: 'Rank Status', value: (profile?.level || 1) >= 10 ? 'Placement Ready' : 'Candidate', icon: Trophy, color: 'text-emerald-500' }
         ].map((stat, i) => (
